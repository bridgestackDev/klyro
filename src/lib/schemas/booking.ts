...
export const createBookingSchema = z.object({
  ...
  clientPhone: z.string().regex(/^\+\d{7,15}$/, "clientPhone must be in E.164 format (e.g. +50422345678)").optional(),
  clientEmail: z.string().email("clientEmail must be a valid email address").optional(),
  notes: z.string().max(500, "notes must be 500 characters or fewer").optional(),
}).refine(
  (data) => Boolean(data.clientPhone) || Boolean(data.clientEmail),
  {
    message: "AT_LEAST_ONE_CONTACT",
    path: ["clientPhone"],
  }
);
